...
class App extends Component {
	...
	fetchQuote = () => {
		...
		// fetches the data 
		axios.get(`http://quotesondesign.com/wp-json/posts?filter[orderby]=rand&filter[posts_per_page]=${posts}`)
  			.then((response) => {
	...
	tweetQuote = () => {
		...
		const win = window.open(`https://twitter.com/intent/tweet?text=${this.state.quote}`, "_blank");
			...
			<div className="app">
				<div className="quote-wrapper">
					<section>
						...
						<p className="quote">{this.state.loading ? "" : this.state.quote}</p>
					</section>
					...
					<footer>Quotes provided by the <a href="https://quotesondesign.com/api-v4-0/" rel="noopener noreferrer" target="_blank"> quotesondesign </a> API.</footer>
				</div>
			</div>
		);
	}
